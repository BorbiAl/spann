		...
		<>
			...
			>
				...
				<div className={`flex flex-col ${isSelf ? "items-end" : "items-start w-full"}`}>
					...
					{/* Reaction pills */}
					<div className={`flex gap-1 mt-1.5 ${isSelf ? "justify-end" : "justify-start pl-1"}`}>
						{reactions.length > 0 &&
							reactions.map((reaction) => {
								...
								return (
									<div
										...
										className="bg-[#F0F0F5] border border-black/[0.05] rounded-[6px] px-1.5 py-0.5 flex items-center gap-1 cursor-pointer hover:bg-[#E5E5EA] active:scale-95 transition-[background-color,transform] duration-[120ms]"
									...
									>
										...
										<span className="text-[11px] font-semibold text-[#1D1D1F] opacity-60 leading-none">{parsed.count}</span>
									</div>
								);
							})}

						{isSelf && (
							<div className="bg-[#F0F0F5] border border-black/[0.05] rounded-[6px] px-1.5 py-0.5 flex items-center gap-1 cursor-pointer hover:bg-[#E5E5EA] active:scale-95 transition-[background-color,transform] duration-[120ms]">
								<span className="text-[10px] leading-none">?</span>
								<span className="text-[10px] font-semibold text-[#1D1D1F] opacity-60 leading-none">2</span>
							</div>
						)}
					</div>
				</div>
			...
			{/* Explain modal — portal to body */}
			{isExplainOpen && understoodResult && (
				<ExplainModal data={understoodResult} onClose={closeExplain} />
			)}
		</>
	);
}
...
function LoadingDots() {
	return (
		<span className="flex items-center gap-0.5 px-1">
			...
			<span className="w-1 h-1 rounded-full bg-current opacity-60 animate-pulse" style={{ animationDelay: "0.15s" }} />
			<span className="w-1 h-1 rounded-full bg-current opacity-60 animate-pulse" style={{ animationDelay: "0.3s" }} />
		</span>
	);
}
